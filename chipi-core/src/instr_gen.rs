...
fn collect_fields(def: &ValidatedDef) -> (Vec<FieldAccessor>, Vec<String>) {
    let fields_iter = def.instructions.iter().map(|i| &i.resolved_fields);
    collect_fields_from(fields_iter, def.config.width, def.config.bit_order)
}

/// Collect fields from a set of instructions' resolved fields.
fn collect_fields_from<'a>(
    instructions: impl Iterator<Item = &'a Vec<ResolvedField>>,
    width: u32,
    bit_order: BitOrder,
) -> (Vec<FieldAccessor>, Vec<String>) {
...
///
/// Produces a `pub struct Name(pub u8/u16/u32)` with field accessor methods,
/// just like [`generate_instr_type`] does for the main decoder.
pub fn generate_subdecoder_instr_type(
    sd: &ValidatedSubDecoder,
    struct_name: &str,
) -> (String, Vec<String>) {
    let fields_iter = sd.instructions.iter().map(|i| &i.resolved_fields);
    let (fields, warnings) = collect_fields_from(fields_iter, sd.width, sd.bit_order);

    let backing_type = width_to_type(sd.width);
    ...
    writeln!(out, "#[derive(Clone, Copy)]").unwrap();
    writeln!(out, "pub struct {}(pub {});", struct_name, backing_type).unwrap();
    writeln!(out).unwrap();
    writeln!(out, "#[rustfmt::skip]").unwrap();
    writeln!(out, "impl {} {{", struct_name).unwrap();

    for field in &fields {
        let body = accessor_body(field, "self.0", sd.width);
        writeln!(out, "    /// Field: `{}`", field.chipi_type).unwrap();
        writeln!(
            out,
            "    #[inline] pub fn {}(&self) -> {} {{ {} }}",
    ...
    writeln!(out).unwrap();
    writeln!(out, "    /// Construct from a raw {} value.", backing_type).unwrap();
    writeln!(out, "    #[inline]").unwrap();
    writeln!(
        out,
        "    pub fn new(val: {}) -> Self {{ Self(val) }}",
        backing_type
    )
    .unwrap();
    writeln!(out, "}}").unwrap();
