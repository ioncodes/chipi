...
impl<'a> Ctx<'a> {
    ...
    fn param_name(&self) -> &'static str {
        if is_primitive(self.instr_type) { "opcode" } else { "instr" }
    }
...
pub fn generate_lut_code(
    ...
    let raw_expr = raw_expr.unwrap_or_else(|| {
        if is_primitive(instr_type) { "opcode" } else { "instr.0" }
    });
    ...
    // OP_* constants. One per instruction, index-stable as long as the spec doesn't change.
    writeln!(out, "// Per-instruction constants. Use as const-generic arguments:").unwrap();
    writeln!(out, "// `fn alu<const OP: u32>(ctx, instr) {{ match OP {{ OP_ADDI => ... }} }}`").unwrap();
...
pub fn generate_stubs_code(
    ...
    let it = instr_type;
    let pn = if is_primitive(instr_type) { "_opcode" } else { "_instr" };

    let mut out = String::new();
    writeln!(out, "// Handler stubs. Implement each function and remove the todo!()").unwrap();
    ...
    for group in emitted_groups {
        ...
        writeln!(out, "pub fn {group}<const OP: u32>(_ctx: &mut {ctx_type}, {pn}: {it}) {{").unwrap();
        writeln!(out, "    match OP {{").unwrap();
        for instr in instrs {
            writeln!(out, "        {} => todo!(\"{instr}\"),", op_const_name(instr)).unwrap();
        }
