...
use chipi_core::compute::{infer_width_fields, infer_width_locals};
...
/// semantics). `inst` resolves operand names. A computed operand becomes `<instr>__<name>(word)`,
/// otherwise it becomes a bound field accessor `<ident>(word)`.
pub fn emit_cond(
    e: &Expr,
    inst: &Insn,
    ...
    match e {
        ...
        Expr::Unary { op, rhs, .. } => {
            let r = emit_cond(rhs, inst, accmap);
        ...
        Expr::Binary { op, lhs, rhs, .. } => cond_binop(
            *op,
            &emit_cond(lhs, inst, accmap),
            &emit_cond(rhs, inst, accmap),
        ),
        Expr::Cond {
            cond, then, els, ..
        } => format!(
            "(({}) if ({}) != 0 else ({}))",
            emit_cond(then, inst, accmap),
            emit_cond(cond, inst, accmap),
            emit_cond(els, inst, accmap)
        ),
